...
class DQN:
    ...
    def __init__(self, env, params):
        ...
        self.eps_delta = (params.init_eps - params.final_eps) \
            / params.final_eps_frame
        ...
        if params.lr_anneal:
            self.lr = tf.train.exponential_decay(
                          params.lr, self.global_step, params.lr_anneal, 0.96,
                          staircase=True)
        else:
            self.lr = params.lr
        ...
        self.optimizer = tf.train.RMSPropOptimizer(
                                    self.lr, params.decay_rate, 0.0, self.eps)

        self.actions = tf.placeholder(tf.float32, [None,
                                                   self.num_actions])
        self.q_target = tf.placeholder(tf.float32, [None])
        self.q_train = tf.reduce_max(tf.multiply(
                        self.train_net.y, self.actions), reduction_indices=1)
        ...
        if params.clip_delta > 0:
            ...
            self.diff_square = tf.multiply(half, tf.add(quadratic_part,
                                                        linear_part))
        else:
    ...
    def observe(self, exploration_rate):
        if rand.random() < exploration_rate:
            ...
            action_values = \
                self.train_net.y.eval(feed_dict={self.train_net.x: x})
    ...
    def doMinibatch(self, sess, successes, failures):
        ...
        state = np.array([batch[i][0] for i in
                         range(self.batch_size)]).astype(np.float32)
        actions = np.array([batch[i][1] for i in
                           range(self.batch_size)]).astype(np.float32)
        rewards = np.array([batch[i][2] for i in
                           range(self.batch_size)]).astype(np.float32)
        successes += np.sum(rewards == 1)
        next_state = np.array([batch[i][3] for i in
                              range(self.batch_size)]).astype(np.float32)
        terminals = np.array([batch[i][4] for i in
                             range(self.batch_size)]).astype(np.float32)

        failures += np.sum(terminals == 1)
        q_target = \
            self.target_net.y.eval(feed_dict={self.target_net.x: next_state})
    ...
    def play(self):
        ...
        for i in xrange(self.episodes):
            ...
            while not terminal:
                ...
                (state, action, reward, screen, terminal) = \
                    self.observe(self.eps)

    def copy_weights(self, sess):
        for key in self.train_net.weights.keys():
            t_key = 'target/' + key.split('/', 1)[1]
            sess.run(self.target_net.weights[t_key].assign(
                                                self.train_net.weights[key]))
