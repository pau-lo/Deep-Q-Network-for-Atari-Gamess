...
class ConvNet:
    ...
    def __init__(self, params, trainable):
        self.shape = [None, params.width, params.height,
    ...
    def max_pool(self, x, size):
        return tf.nn.max_pool(x, ksize=[1, size, size, 1], strides=[1,
                              size, size, 1], padding='SAME')
    ...
    def infer(self, _input):
        ...
        for layer in range(self.num_layers):
            with tf.variable_scope('conv' + str(layer)) as scope:
                ...
                shape = [self.filter_size[layer],
                         self.filter_size[layer], in_channels,
                         out_channels]

                w = self.create_weight(shape)
                conv = self.create_conv2d(self.layers[-1], w,
                                          self.filter_stride[layer])
        ...
        for layer in range(self.fc_layers):
            with tf.variable_scope('hidden' + str(layer)) as scope:
                ...
                self.weights[w.name] = w
                self.weights[b.name] = b
                hidden = tf.nn.relu_layer(self.layers[-1], w, b,
                                          name=scope.name)
